...
function removeAndDouble(arr, startIdx) {
  ...
  const arr1 = arr.splice(0, startIdx);
  const doubledArray = arr1.map(x => x * 2);
  return doubledArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const filteredSquaredArray = map1.filter(value => value > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const lastNElements = arr.slice(startIndex);
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((value, index) => arr.indexOf(value) !== index);
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((accumulator, item) => accumulator + item.price, 0);
  return totalPrice;
...
function findLastNegativeNumberIndex(arr) {
  ...
  const num1 = (element) => element < 0;
  return arr.findLastIndex(num1);
